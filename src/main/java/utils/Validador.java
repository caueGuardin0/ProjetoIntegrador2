...
public class Validador {
    ...
    public void ValidarComboBox(JComboBox slc) {
        try {
            // Verifico se existe algum seleção
            if (slc.getSelectedIndex() == 0) {
                throw new IllegalArgumentException();
            }

            String selecao = slc.getSelectedItem().toString();
            slc.setBackground(Color.WHITE);

        } catch (IllegalArgumentException e) {
            this.mensagensErro.add("Selecione um campo para o: " + slc.getName());
            slc.setBackground(Color.red);
        }
    }
